...
SUCCESS_BC_WINDOW="${SUCCESS_BC_WINDOW:-64}"
SUCCESS_BUFFER_CAPACITY="${SUCCESS_BUFFER_CAPACITY:-4096}"
SUCCESS_BC_BATCH_SIZE="${SUCCESS_BC_BATCH_SIZE:-64}"
SUCCESS_BC_WARMUP_EPISODES="${SUCCESS_BC_WARMUP_EPISODES:-0}"
SUCCESS_BC_EPISODE_LEN="${SUCCESS_BC_EPISODE_LEN:-150}"
SUCCESS_BC_RECENCY_HALF_LIFE="${SUCCESS_BC_RECENCY_HALF_LIFE:-0}"
SUCCESS_BC_CRITIC_GATE="${SUCCESS_BC_CRITIC_GATE:-false}"
SUCCESS_BC_CRITIC_GATE_MODE="${SUCCESS_BC_CRITIC_GATE_MODE:-batch}"
SUCCESS_BC_CRITIC_STATE_NOISE="${SUCCESS_BC_CRITIC_STATE_NOISE:-0.1}"
ACTOR_GOAL_MODE="${ACTOR_GOAL_MODE:-her}"
ACTOR_SUCCESS_SCORE_WEIGHT="${ACTOR_SUCCESS_SCORE_WEIGHT:-0.0}"
TRUNCATE_ON_SUCCESS="${TRUNCATE_ON_SUCCESS:-false}"
...
build_flags() {
  ...
  if [ -n "${RESUME_CHECKPOINT_FILE:-}" ]; then
    _FLAGS="$_FLAGS --resume_checkpoint_file=$RESUME_CHECKPOINT_FILE"
  fi
  ...
  _FLAGS="$_FLAGS --success_bc_window=$SUCCESS_BC_WINDOW"
  _FLAGS="$_FLAGS --success_buffer_capacity=$SUCCESS_BUFFER_CAPACITY"
  _FLAGS="$_FLAGS --success_bc_batch_size=$SUCCESS_BC_BATCH_SIZE"
  _FLAGS="$_FLAGS --success_bc_warmup_episodes=$SUCCESS_BC_WARMUP_EPISODES"
  _FLAGS="$_FLAGS --success_bc_episode_len=$SUCCESS_BC_EPISODE_LEN"
  _FLAGS="$_FLAGS --success_bc_recency_half_life=$SUCCESS_BC_RECENCY_HALF_LIFE"
  if [ "$SUCCESS_BC_CRITIC_GATE" = "true" ]; then
    _FLAGS="$_FLAGS --success_bc_critic_gate"
  else
    _FLAGS="$_FLAGS --nosuccess_bc_critic_gate"
  fi
  _FLAGS="$_FLAGS --success_bc_critic_gate_mode=$SUCCESS_BC_CRITIC_GATE_MODE"
  _FLAGS="$_FLAGS --success_bc_critic_state_noise=$SUCCESS_BC_CRITIC_STATE_NOISE"
  _FLAGS="$_FLAGS --actor_goal_mode=$ACTOR_GOAL_MODE"
  _FLAGS="$_FLAGS --actor_success_score_weight=$ACTOR_SUCCESS_SCORE_WEIGHT"
  if [ "$TRUNCATE_ON_SUCCESS" = "true" ]; then
    _FLAGS="$_FLAGS --truncate_on_success"
  else
    _FLAGS="$_FLAGS --notruncate_on_success"
  fi
...
for ((i = 0; i < TASKS_PER_GPU; i++)); do
  ...
  (
    ...
    echo "Action effect   : enabled=$ACTION_EFFECT_ENABLED actor=$ACTION_EFFECT_ACTOR_MODE target=$ACTION_EFFECT_TARGET_MODE H=$OUTCOME_HORIZON bc=$SUCCESS_BC_WEIGHT truncate=$TRUNCATE_ON_SUCCESS"

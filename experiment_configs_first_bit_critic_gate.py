#!/usr/bin/env python3
"""Two Success-BC fixes on Tasks 4/7 and 5/8 (8 jobs).

Fix 1 (cells 0-3): insert only the first sparse-bit=1 transition.
Fix 2 (cells 4-7): current-sparse inserts; scale NLL by how flat the
DCC critic is across random actions at that observed (s, g). The stored
success action is the cloning target only, never a ranking oracle.

Same λ=0.1, same 4096 ring, paper stack (native_info, dyn=0, 1024×4).
No hover radius, no task list, no extra head.
"""
...
SEED = 6
WANDB_GROUP = 'PAPER-DCC-FIRSTBIT-CRITICGATE-4758'
PAPER_BC_DIR = (
...
def _task_cells(label_mode, critic_gate, suffix):
  return [
      _paper(
          8_000_000, 100_000,
          variant=f'task4_stick_{suffix}',
          start_task=4,
          num_tasks=5,
          adapt_heads_only=True,
          resume_checkpoint_file=f'{PAPER_BC_DIR}/seed_{SEED}/task_3.pkl',
          success_bc_label_mode=label_mode,
          success_bc_critic_gate=critic_gate,
      ),
      _paper(
          8_000_000, 100_000,
          variant=f'task7_shelf_{suffix}',
          start_task=7,
          num_tasks=8,
          adapt_heads_only=True,
...
def build_configs():
  return (
      _task_cells('first_sparse_bit', False, 'first_bit')
      + _task_cells('current_sparse_reward', True, 'critic_gate')
  )
  ...
  if args.list:
    for i, c in enumerate(configs):
      ...
      print(i, c['variant'], task,
            ...
            f"gate={c['success_bc_critic_gate']}",

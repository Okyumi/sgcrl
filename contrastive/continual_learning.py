...
class ContinualContrastiveLearner(acme.Learner):
  ...
  def __init__(
    ...
    def _compose_theta(policy_base, pool_vectors_flat, beta_k, alpha_scale, v_k):
      """Compose θ' inside a JIT-able function.

      pool_vectors_flat is a *stacked* array representation that we unstack
      into a list.  However, because the pool can have variable length and
      JAX's JIT requires static shapes, we use a simpler approach: we
      pre-compose the weighted pool contribution *outside* JIT and pass it
      as a single pytree ``pool_contribution``.
      """
      # This function is NOT called directly.  See _make_update_step.
      pass

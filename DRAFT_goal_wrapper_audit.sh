...
# Gate V0: no training.  A GPU is required so mujoco_py uses EGL instead of
# compiling the OSMesa CPU shim (GL/osmesa.h is not present on Torch CPU
# nodes).  Every seed must pass before V1.

#!/bin/bash
#SBATCH --verbose
#SBATCH --job-name=jubail_t56_vid
...
# Task-5 / Task-6 visual rollouts (GIFs).
#   0  DCC w/o BC, handle-press: 10 paced GIFs + first-success hunt
#      from existing seed-6 mid-ckpts (job 17901349)
#   1  DCC push: 10 paced GIFs from the same job
#   2  DCC + Success-BC handle-press: train 1M, save paced + first-success
...
python -c "import experiment_configs_jubail_task_videos as c; assert len(c.build_configs())==3"
...
echo "Jubail Task-5/6 video cell ${CONFIG_IDX} (${VARIANT}) finished."

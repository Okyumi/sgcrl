#!/usr/bin/env python3
"""Resume Task 4 / Task 7 with terminal-episode Success-BC.

Loads the paper Success-BC predecessors (seeds 5–14, ``task_3.pkl`` /
``task_6.pkl``) and trains only the next task. The success buffer gets
every transition of an episode iff the last sparse reward is 1. Episodes
that hit then undo are not cloned. Eval is unchanged (any in-episode
sparse hit still counts). Use the 10-seed learning curves for paper AUC
and best success on these two tasks.

Cells:
  0–9   stick_pull from task_3, seeds 5–14, 8M
  10–19 shelf_place from task_6, seeds 5–14, 8M
"""
...
WANDB_GROUP = 'PAPER-DCC-TERMINAL-BC-TASK47'
PAPER_BC_DIR = (

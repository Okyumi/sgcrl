...
#SBATCH --mem=96GB
#SBATCH --output=/scratch/yd2247/sgcrl/logs/paper_first_seeds/%A_%a.out
...
#   sbatch --array="$ARRAY" DRAFT_paper_first_seeds.sh
set -euo pipefail
...
mkdir -p "$LOG_DIR" "$CHECKPOINT_DIR" \
    /scratch/yd2247/sgcrl/logs/paper_first_seeds

python tests/test_paper_first_seeds.py

ARRAY_TASK_ID="${SLURM_ARRAY_TASK_ID:-0}"
if python scripts/paper_first_seeds_status.py \
    --array-task-complete \
...
if [ -n "${SLURM_JOB_ID:-}" ] && [ "$CHAIN_INDEX" -lt "$MAX_CHAIN" ]; then
  ...
  NEXT_JOB="$(sbatch --parsable \
      ...
      --array="${ARRAY_TASK_ID}" \
      --export=ALL,PAPER_FIRST_SEEDS_CHAIN_INDEX="${NEXT_CHAIN}",PAPER_FIRST_SEEDS_MAX_CHAIN="${MAX_CHAIN}" \
      "$REPO_DIR/DRAFT_paper_first_seeds.sh" || true)"
  if [ -n "$NEXT_JOB" ]; then
...
echo "Paper first-seeds chain index: ${CHAIN_INDEX}/${MAX_CHAIN}"
bash "$REPO_DIR/DRAFT.sh"

if [ -n "$NEXT_JOB" ] && python scripts/paper_first_seeds_status.py \
    --array-task-complete \
    --array-task-id="$ARRAY_TASK_ID" \
    --tasks-per-gpu="$TASKS_PER_GPU" \
    --offset="$CONFIG_INDEX_OFFSET" \

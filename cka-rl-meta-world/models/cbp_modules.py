...
class cbpshared(nn.Module):
    ...
    def setup_feature_logging(self, h_dim) -> None:
        ...
        """
        self.activations = {}
        self.feature_keys = [self.net[i * 2 + 1] for i in range(len(h_dim))]


        # Register "hook_fn" for each layer
        register_hook(self.net, self.hook_fn)
    ...
    def remove_hooks(self):
        for hook in list(self.net._forward_hooks.values()):
            self.net._forward_hooks.pop(hook)
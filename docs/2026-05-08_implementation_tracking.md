...
| D7 | CKA diagnostic run on `actor_mode='cka', critic_mode='cka'` with `log_pool_cosine=True` | A | 3 seeds run 2026-05-12/13; analysed 2026-05-14 | Actor mixture decays to ~0.02; critic mixture stays at ~0.5; critic α_scale GROWS on hard tasks (up to 2.0 on k=8). Mechanism identified by H3 test (`2026-05-14_h3_logsumexp_test.md`): critic α_scale is driven by `logsumexp_penalty` only on tasks where InfoNCE logits spike (k=5, k=8); these are exactly the tasks where the CKA cell fails. H1 test (`2026-05-14_h1_h2_alpha_predicts_failure.md`): critic α_scale is anti-correlated with success (Pearson r=-0.54, p=0.007). Conclusion: the critic mixture is a numerical-stability symptom, not a knowledge-transfer channel. Both original "vectors collapse" and 2026-05-12 "null space" hypotheses refuted. |

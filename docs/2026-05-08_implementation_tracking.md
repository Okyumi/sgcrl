...
| D7 | CKA diagnostic run on `actor_mode='cka', critic_mode='cka'` with `log_pool_cosine=True` | A | 3 seeds run 2026-05-12/13 | actor mixture decays to ~0.02; critic mixture decays only to ~0.5 (NOT zero); critic α_scale GROWS on later tasks (up to 2.0). Both original "vectors collapse" and revised "gradient null space" hypotheses refuted. The failure is asymmetric actor-vs-critic. See `docs/2026-05-13_wandb_findings.md` and `docs/2026-05-13_revised_hypothesis_plan.md`. |

...
| D7 | CKA diagnostic run on `actor_mode='cka', critic_mode='cka'` with `log_pool_cosine=True` | A | one seed run 2026-05-12 | `mixture_norm` collapse confirmed (12 -> 0 critic, 0.25 -> 0 actor across tasks 2-9); `pool_cos_mean_offdiag` ~0.17 actor / ~0 critic, NOT >0.9 as conjectured. Original "vectors collapse" hypothesis refuted; revised hypothesis in `docs/2026-05-12_cka_failure_results.md`. One more seed needed for reproducibility. |
